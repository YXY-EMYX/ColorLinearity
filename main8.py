#通过偏移到指定颜色色环直径实现图片色调的调换，增加角度限制，超出限制依旧会偏移一定角度（修复了7的问题）
...
def adjust_hue_within_limit(hue, target_hue, limit=0.0277):  # limit 10度
    ...
    if distance_to_target < distance_to_opposite:
        # 朝向目标色调偏转
        adjusted_hue = target_hue if distance_to_target <= limit else (hue + limit) % 1 if hue < target_hue or hue - target_hue > 0.5 else (hue - limit) % 1
    else:
        # 朝向相反色调偏转
        opposite_target_hue = (target_hue + 0.5) % 1
        adjusted_hue = opposite_target_hue if distance_to_opposite <= limit else (hue + limit) % 1 if hue < opposite_target_hue or hue - opposite_target_hue > 0.5 else (hue - limit) % 1
