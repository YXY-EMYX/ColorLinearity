#通过偏移到色段的颜色色环实现图片色调的调换，采用了极坐标变换
import numpy as np
import colorsys
...
def find_closest_point_on_line(hsv_color, line_points):
    # 将色调转换为极坐标系中的角度
    angle = hsv_color[0] * 2 * np.pi
    saturation = hsv_color[1]

    # 色环上的点坐标
    x, y = saturation * np.cos(angle), saturation * np.sin(angle)
    ...
    # 将坐标转换回色调
    closest_angle = np.arctan2(closest_y, closest_x)
    closest_hue = closest_angle / (2 * np.pi) % 1
    return closest_hue, hsv_color[1], hsv_color[2]
