#通过偏移到指定颜色色环直径实现图片色调的调换，添加了饱和度权重，饱和度较低的颜色对最佳直径的确定有更大的影响。这意味着在调整图像色调时，这些饱和度低的颜色相比于其他颜色变化更小。
...
# 计算总距离，考虑饱和度的影响
def total_distance_with_inverted_saturation_weight(hsv_colors, line_angle):
    max_saturation = max(s for h, s, v in hsv_colors)  # 找出最高饱和度
    total_distance = 0
    line_hue = line_angle / 360
    for h, s, v in hsv_colors:
        weight = max_saturation - s  # 使用最高饱和度减去当前饱和度作为权重
        distance_to_line = hue_distance(h, line_hue) * weight
        distance_to_opposite_line = hue_distance(h, (line_hue + 0.5) % 1) * weight
...
# 在主函数中使用新的函数来找到最佳直径
result = minimize(lambda x: total_distance_with_inverted_saturation_weight(hsv_colors, x), x0=0, bounds=[(0, 360)])
